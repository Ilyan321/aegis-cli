...
Write-Host "🛡️ Installing Aegis CLI on Windows..." -ForegroundColor Cyan
...
Write-Host "⬇️ Downloading Aegis from GitHub Releases..." -ForegroundColor Cyan
try {
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    Invoke-WebRequest -Uri $ReleaseUrl -OutFile $ZipPath -UseBasicParsing
} catch {
    Write-Host "⚠️ Precompiled release not yet found. Please install Go (https://go.dev) or build from source:" -ForegroundColor Yellow
...
Write-Host "`n🎉 Aegis CLI installed successfully!" -ForegroundColor Green
Write-Host "Run 'aegis --help' to get started." -ForegroundColor Yellow
